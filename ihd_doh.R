...
# beta.groups <- c(0.02311117)  #	(0.013102826 - 0.033647224)             # Lepuele et al. 2012 
beta.groups <- c(0.01310282624)# , 0.00198026273, 0.01988508587)
  names(beta.groups) <- c('PE_ihd')# , 'LCI_ihd', 'UCI_ihd') #,'LCI','UCI')

rate.groups <- c('ihd_15_doh')              # CHANGE YEAR
  names(rate.groups) <- c('IHD_15')         # CHANGE YEAR
...
getwd()
setwd("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ihd_doh/")
    ...
    for (k in 1:length(rate.groups)){
      ...
      for (h in 1:length(pop.groups)){
        ...
        for (m in 1:length(city.groups)){
          ...
          # file.exists("/Users/mdcc/Box/MC_R/HIA/run/rates_dc/ihd_25_2015_dc.tif") # check whether file exists in folder
          ...
          #==============================================================================================================================================================    
          # f1 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/ihd_doh/adults/af/',names(city.groups[m]),names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f1 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ihd_doh/af/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' AF IHD and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('af')
          ...
          f2 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ihd_doh/mr/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' MR IHD and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('mr')
          ...
          # f3 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/ihd_doh/adults/paf/',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f3 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ihd_doh/paf/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' PAF IHD and PM25 excess cases, ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('hia')
          ...
          for (s in 1:length(clip.groups)){
            ...
            e_rate <- ggplot()  +
              ...
              geom_path(data = clips.shp.f, aes(x = long, y = lat, group = group),
                        color = "grey60", size = 0.3)+
              labs(title=bquote('IHD Excess Mortality Attributable to '~PM[2.5]~' (2015)'),
                   caption=paste0(names(beta.groups[i]),', ',names(conc.groups[j]),', \n',names(rate.groups[k]),', ',names(pop.groups[h]),'.',sep=''),
            ...
            print(e_rate3)
            ggsave(paste('IHD_comp_color ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(rate.groups[k]),'.png',sep=''),
            ...
            e_rate2 <- ggplot() +
              ...
              geom_path(data = clips.shp.f, aes(x = long, y = lat, group = group),
                        color = "grey60", size = 0.5) +
              # labs(title=bquote('IHD')) #,
              labs(title=paste0('IHD (2015)'),
                   subtitle=paste0('Range: ',rate.min.label,' - ',rate.max.label,' per 100,000'),sep='') + 
              theme (plot.title = element_text(face="bold", hjust = 0.5, size = 23),
                     plot.subtitle = element_text(face = "italic", hjust = 0.5, size = 21))
            print(e_rate2)
            ggsave(paste('IHD_composite ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(rate.groups[k]),'.png',sep=''),dpi=300)
            print("IHD")
          }
