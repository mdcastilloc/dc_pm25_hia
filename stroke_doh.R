...
beta.groups <- c(0.01043600153) #, 0.004879016417, 0.01570037488)  
  names(beta.groups) <- c('PE_str') #,'LCI_str','UCI_str')   

rate.groups <- c('str_15_doh')              # CHANGE YEAR
  names(rate.groups) <- c('STR_15')         # CHANGE YEAR
...
getwd()
setwd("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/str_doh/")
      ...
      for (h in 1:length(pop.groups)){
        ...
        for (m in 1:length(city.groups)){
          ...
          b = raster(paste(rates,rate.groups[k],'.tif',sep=''))	
            ...
            # file.exists("/Users/mdcc/Box/MC_R/HIA/run/rates_dc/str_25_2015_dc.tif") # check whether file exists in folder
          ...
          #==============================================================================================================================================================    
          # f1 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/str_doh/adults/af/',names(city.groups[m]),names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f1 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/str_doh/af/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' AF STR and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('af')
          ...
          f2 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/str_doh/mr/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' MR STR and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('mr')
          ...
          # f3 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/str_doh/adults/paf/',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f3 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/str_doh/paf/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          paf_plot <- ggplot()  +
            ...
            geom_path(data = shp.f, aes(x = long, y = lat, group = group), 
                      color = "grey60", size = 0.5)+
            labs(title=bquote('Stroke Excess Mortality Attributable to '~PM[2.5]~''),
          ...
          for (s in 1:length(clip.groups)){
            ...
            e_rate <- ggplot()  +
              ...
              geom_path(data = clips.shp.f, aes(x = long, y = lat, group = group),
                        color = "grey60", size = 0.3)+
              labs(title=bquote('Stroke Excess Mortality Attributable to '~PM[2.5]~' (2015)'),
                   caption=paste0(names(beta.groups[i]),', ',names(conc.groups[j]),', \n',names(rate.groups[k]),', ',names(pop.groups[h]),'.',sep=''),
            ...
            print(e_rate3)
            ggsave(paste('Str_comp_color ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(rate.groups[k]),'.png',sep=''),
            ...
            e_rate2 <- ggplot() +
              ...
              theme (plot.title = element_text(face="bold", hjust = 0.5, size = 23),
                     plot.subtitle = element_text(face = "italic", hjust = 0.5, size = 21))
            print(e_rate2)
            ggsave(paste('Str_composite ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(rate.groups[k]),'.png',sep=''),dpi=300)
            print("Str")
          }
