# ==========================================================================================================================================
#
# HIA, DC, Asthma, DOH data, 2014 - 2018
# *** only need to change the date in 4 places and rerun the loops *** 
# ALSO, change the population dataset to all-ages, and add zip codes clip
# Started: 08032020
# 
# ==========================================================================================================================================
...
clip.groups <- c('png_dc','ward_dc') #,'zip_dc')        # 'tract_dc',
  names(clip.groups) <- c('PNGs','Wards') #,'Zip')   # 'Tracts',
...
beta.groups <- c(0.005602959022) #, 0.001421475836, 0.009665521211) # Mar et al
    names(beta.groups) <- c('PE_ast') #, 'LCI_ast', 'UCI_ast') #,'LCI','UCI')


rate.groups <- c('ast_15_doh')              # CHANGE YEAR
  names(rate.groups) <- c('AST_15')         # CHANGE YEAR
...
getwd()
setwd("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ast_doh/")
    ...
    for (k in 1:length(rate.groups)){
      ...
      for (h in 1:length(pop.groups)){
        ...
        for (m in 1:length(city.groups)){
          ...
          # file.exists("/Users/mdcc/Box/MC_R/HIA/run/rates_dc/ast_25_2015_dc.tif") # check whether file exists in folder
          ...
          #==============================================================================================================================================================    
          # f1 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/ast_doh/adults/af/',names(city.groups[m]),names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f1 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ast_doh/af/",names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          print(paste(names(city.groups[m]),' Morbidity Risk ',names(conc.groups[j]),' ',names(beta.groups[i]),' ',names(rate.groups[k]),sep=''))
          print(df.mr)
          
          f2 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ast_doh/mr/",names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          # f3 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/ast_doh/adults/paf/',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f3 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ast_doh/paf/",names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          print(paste(names(city.groups[m]),' excess morbidity ',names(conc.groups[j]),' ',names(beta.groups[i]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),sep=''))
          print(df.hia)
          ...
          paf_plot <- ggplot()  +
            ...
            geom_path(data = shp.f, aes(x = long, y = lat, group = group), 
                      color = "grey60", size = 0.5)+
            labs(title=bquote('Asthma Excess Morbidity Attributable to '~PM[2.5]~''),
          ...
          for (s in 1:length(clip.groups)){
            ...
            e_rate <- ggplot()  +
              ...
              geom_path(data = clips.shp.f, aes(x = long, y = lat, group = group),
                        color = "grey60", size = 0.3)+
              labs(title=bquote('Asthma Excess Morbidity Attributable to '~PM[2.5]~' (2015)'),
                   caption=paste0(names(beta.groups[i]),', ',names(conc.groups[j]),', \n',names(rate.groups[k]),', ',names(pop.groups[h]),'.',sep=''),
            ...
            print(e_rate3)
            ggsave(paste('Ast_comp_color ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(rate.groups[k]),'.png',sep=''),
            ...
            e_rate2 <- ggplot()  +
              ...
              theme (plot.title = element_text(face="bold", hjust = 0.5, size = 23),
                     plot.subtitle = element_text(face = "italic", hjust = 0.5, size = 21))
            print(e_rate2)
            ggsave(paste('Asthma_composite_',names(clips.shp[s]),names(beta.groups[i]),'_',names(rate.groups[k]),'.png',sep=''),dpi=300)
            print("asthma")
          }
