# ==========================================================================================================================================
#
# HIA, DC, AST, 2011-2015 average PM2.5, pop and Rates/counts
# replace PM2.5 dataset with that of the 5-year average (for ast 2011-2015)
# replace the rates file with that of the 5-year average counts
# update HIA = 1-exp(-B * conc) * counts
# comment out pop from the input groups
# update filepath for outputs with /ast_five/
# remove the years (eg 2014) from the title of the plots
# leave the c (pop) file for the 
# 
# ==========================================================================================================================================
...
counts <- '/Users/mdcc/Box/dc_hia/HIA_calculations_mc/run/counts_dc/'
...
# comment out pop.groups for the HIA calculation (delta morbidity) when using counts instead of rates
...
beta.groups <- c(0.005602959022) 
  names(beta.groups) <- c('PE_ast') #, 'LCI_ast', 'UCI_ast') #,'LCI','UCI')
count.groups <- c('ast_14_18_doh')             # update to 5-year average counts
  names(count.groups) <- c('AST_14_18_DOH')    
conc.groups <- c('vdk_14_18_dc')             # update for 5-year average: 2011 - 2015
  names(conc.groups) <- c('DC_14-18')          


pdf(NULL)

getwd()
setwd("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ast_doh/ast_five_ave/")
...
for (j in 1:length(conc.groups)){
  ...
  for (i in 1:length(beta.groups)){
    ...
    for (k in 1:length(count.groups)){
      print(count.groups[k]) 
      
      for (h in 1:length(pop.groups)){   # note: remove the '}' at the end 
        print(pop.groups[h])
        ...
        for (m in 1:length(city.groups)){
          ...
          b = raster(paste(counts,count.groups[k],'.tif',sep=''))	
          
          c = raster(paste(pops,pop.groups[h],'.tif',sep=''))
          c[c==0]<-NA
          
          mr <- (af2 * b * 10000 / c)    # betas*conc * rates (counts * 10,000/pop) 
          mr[mr==0]<-NA
          ...
          # update hia function with counts instead of rates -> remove c, r1, and 10^-4 
          hia = overlay(b, a, fun=function(r2, r3){return(r2*(1-exp(-beta.groups[i]*r3)))})
          hia[hia==0]<-NA
          ...
          #==============================================================================================================================================================    
          # f1 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/ast_doh/adults/af/',names(city.groups[m]),names(conc.groups[j]),', ',names(count.groups[k]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f1 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ast_doh/ast_five_ave/af/",names(conc.groups[j]),', ',names(count.groups[k]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' AF AST and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(count.groups[k]),'.png',sep=''),
          ...
          print(paste(names(city.groups[m]),' Morbidity Risk ',names(conc.groups[j]),' ',names(beta.groups[i]),' ',names(count.groups[k]),sep=''))
          print(df.mr)
          
          f2 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ast_doh/ast_five_ave/mr/",names(conc.groups[j]),', ',names(count.groups[k]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          writeRaster(mr, filename=f2, format="GTiff", overwrite=TRUE)
          
          # Create maps morbidity risk 
          min.mr <- minValue(mr)
          ...
          mr_plot <- ggplot()  +
            ...
            scale_fill_gradient2("Morbidity Risk per 10,000",
                                 low = "#3ec267", 
                                 mid = "#fff429",  #ff7e29
                                 high = "#fc0339", ##ff1f40
                                 midpoint = mean.mr,
            ...
            geom_path(data = shp.f, aes(x = long, y = lat, group = group), 
                      color = "grey60", size = 0.5)+
            labs(title='Morbidity Risk',
                 caption=paste0(names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(count.groups[k]),'.',sep=''),
                 subtitle=paste0('Range: ',min.mr.label,' to ',max.mr.label,' per 10,000. '),sep='')
          ggsave(paste(names(city.groups[m]),' MR AST and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(count.groups[k]),'.png',sep=''),
                 width = 8, height = 10, dpi = 300, units = "in")
          print('mr')
          ...
          # f3 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/ast_doh/adults/paf/',names(conc.groups[j]),', ',names(count.groups[k]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f3 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/ast_doh/ast_five_ave/paf/",names(conc.groups[j]),', ',names(count.groups[k]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          print(paste(names(city.groups[m]),' excess morbidity ',names(conc.groups[j]),' ',names(beta.groups[i]),' ',names(count.groups[k]),sep=''))
          print(df.hia)
          ...
          paf_plot <- ggplot()  +
            ...
            geom_path(data = shp.f, aes(x = long, y = lat, group = group), 
                      color = "grey60", size = 0.5)+
            labs(title=bquote('AST Excess Morbidity Attributable to '~PM[2.5]~''),
                 caption=paste0(names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(count.groups[k]),',\n','.',sep=''),
                 subtitle=paste0('Range: ',min.hia.label,' to ',max.hia.label,' per grid cell. '),sep='')
          ggsave(paste(names(city.groups[m]),' PAF AST and PM25 excess cases, ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(count.groups[k]),'.png',sep=''),
                 width = 8, height = 10, dpi = 300, units = "in")
          print('hia')
          ...
          for (s in 1:length(clip.groups)){
            ...
            # this brings in the raster of the hia 
            zone.in <- clips.shp
            raster.in <- hia
            ...
            # this brings in the c file, which is the pop dataset
            zone.in <- clips.shp
            raster.in <- c
            ...
            rate_df$rate <- (rate_df$hia.val*10000)/rate_df$pop.val
            ...
            write.csv(rate_df, paste(names(clips.shp[s]),' ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(count.groups[k]),'results.csv'))
            ...
            e <- ggplot()  +
              ...
              labs(title=bquote('AST Excess Morbidity Attributable to '~PM[2.5]~''),
                   caption=paste0(names(beta.groups[i]),', ',names(conc.groups[j]),', \n',names(count.groups[k]),', ','.',sep=''),
                   subtitle=paste0('Range: ',r.min.label,' to ',r.max.label),sep='')
            ggsave(paste(names(clips.shp[s]),' PAF counts, ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(count.groups[k]),'.png',sep=''),
                   width = 8, height = 10, dpi = 300, units = "in")
            print('clips.shp')
            ...
            e_rate <- ggplot()  +
              ...
              scale_fill_gradient2("Rate per 10,000",
                                   low = "#3ec267",
                                   mid = "#fff429",  #ff7e29
                                   high = "#fc0339", ##ff1f40
                                   midpoint = rate.mean,
              ...
              labs(title=bquote('AST Excess Morbidity Attributable to '~PM[2.5]~' '),
                   caption=paste0(names(beta.groups[i]),', ',names(conc.groups[j]),', \n',names(count.groups[k]),'.',sep=''),
                   subtitle=paste0('Range: ',rate.min.label,' to ',rate.max.label,' per 10,000. '),sep='')
            ggsave(paste(names(clips.shp[s]),' PAF rates, ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(count.groups[k]),'.png',sep=''),
                   width = 8, height = 10, dpi = 300, units = "in")
            print('rate.png')
            ...
            e_rate3 <- ggplot() +
              ...
              geom_path(data = clips.shp.f, aes(x = long, y = lat, group = group),
                        color = "grey60", size = 0.5) +
              labs(title=paste0('AST'),
                   subtitle=paste0('Range: ',rate.min.label,' - ',rate.max.label,' per 10,000'),sep='') + 
            ...
            print(e_rate3)
            ggsave(paste('AST_comp_color ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(count.groups[k]),'.png',sep=''),
                   width = 8, height = 10, dpi = 300, units = "in")
            print("AST")
            
            
            # COMPOSITE FOR PAF COUNTS 
            clips.png <- readOGR(dsn=path.expand("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/run/clip_dc"), layer="png_dc")
            crs(clips.png) <- "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
            requireNamespace("rgeos")
            clips.png <- crop(clips.png, shp)    # crop all clips (pngs, tracts and wards) to the extent of shp (boundary_dc)
            ...
            e_count <- ggplot() +
              ...
              scale_fill_gradient2( "Cases (n)", 
                                    ...
                                    guide = guide_colourbar(
                                      ...
                                      label.position = "bottom")) +
              theme_map() + 
              theme(panel.border = element_blank()) +    # override rectangular box for map to none
              geom_path(data = clips.shp.f, aes(x = long, y = lat, group = group),
                        color = "grey60", size = 0.3)+
              labs(title=paste0('AST'),
                   subtitle=paste0('Range: ',r.min.label,' - ',r.max.label),sep='') + 
              theme (plot.title = element_text(face="bold", hjust = 0.5, size = 23),
                     plot.subtitle = element_text(face = "italic", hjust = 0.5, size = 21),
                     legend.title = element_text(size=18), legend.text = element_text(size=18))
            print(e_count)
            ggsave(paste('AST_comp_count ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(count.groups[k]),'.png',sep=''),
                   width = 8, height = 10, dpi = 300, units = "in")
            print("comp_counts")
