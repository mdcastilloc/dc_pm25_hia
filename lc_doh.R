...
beta.groups <- c(0.008617769624) #, 0.002955880224, 0.01484200051)  # Turner et al 2016 (30-99) for Lung Cancer
  names(beta.groups) <- c('PE_lc') #,'LCI_lc','UCI_lc')   
  
rate.groups <- c('lc_15_doh')              # CHANGE YEAR
  names(rate.groups) <- c('LC_15')         # CHANGE YEAR
...
getwd()
setwd("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/lc_doh/")
    ...
    for (k in 1:length(rate.groups)){
      ...
      for (h in 1:length(pop.groups)){
        ...
        for (m in 1:length(city.groups)){
          ...
          # file.exists("/Users/mdcc/Box/MC_R/HIA/run/rates_dc/lc_25_2015_dc.tif") # check whether file exists in folder
          ...
          #==============================================================================================================================================================    
          # f1 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/lc_doh/adults/af/',names(city.groups[m]),names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f1 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/lc_doh/af/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          f2 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/lc_doh/mr/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          # f3 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/lc_doh/adults/paf/',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f3 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/lc_doh/paf/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          for (s in 1:length(clip.groups)){
            ...
            e_rate <- ggplot()  +
              ...
              geom_path(data = clips.shp.f, aes(x = long, y = lat, group = group),
                        color = "grey60", size = 0.3)+
              labs(title=bquote('LC Excess Mortality Attributable to '~PM[2.5]~' (2015)'),
                   caption=paste0(names(beta.groups[i]),', ',names(conc.groups[j]),', \n',names(rate.groups[k]),', ',names(pop.groups[h]),'.',sep=''),
            ...
            print(e_rate3)
            ggsave(paste('LC_comp_color ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(rate.groups[k]),'.png',sep=''),
            ...
            print(e_rate2)
            ggsave(paste('LC_composite ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(rate.groups[k]),'.png',sep=''),dpi=300)
            print("LC")
