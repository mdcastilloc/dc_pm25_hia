# ==========================================================================================================================================
# HIA code for Washington DC 2014, Stroke 
# Started: 03102020
# Data sources: PM25: van Donkelaar 2014, 
# Rates: Alex's BenMAP output shapefiles for 2014 (from CDC WONDER and CDC 500 Cities), 
...
# bring in all data inputs for the analyses
      ...
      # rate.groups <- c('stroke_2014_cdc_100m')                    # doh = rates computed by DOH, wo doh = SEDAC rates
      #   names(rate.groups) <- c('Stroke_CDC')
      # conc.groups <- c('2010_100m_dc','2011_100m_dc','2012_100m_dc','2013_100m_dc','2014_100m_dc','2015_100m_dc')     
      #                                                             # conc updated with 2019 dataset. before: pm25_2014_dc_100m
      #   names(conc.groups) <- c('dc_2010','dc_2011','dc_2012','dc_2013','dc_2014','dc_2015')

  rate.groups <- c('str_01_doh')
    names(rate.groups) <- c('Stroke_01') 
  conc.groups <- c('vdk_2001_dc') 
    names(conc.groups) <- c('DC_01')  
  pop.groups <- c('gpw_30-99_2010_dc_100m')
    names(pop.groups) <- c('GPW_30')
      ...
      #   names(pop.groups) <- c('GPW_30_100m')
...
# beta.groups <- c(0.00953101798, 0.0009950330853, 0.01739533071)     # Mar et al 2010 (0-99) for Asthma
  names(beta.groups) <- c('PE','LCI','UCI')
...
getwd()
setwd("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/stroke/")
    ...
    for (k in 1:length(rate.groups)){
      ...
      for (h in 1:length(pop.groups)){
        ...
        for (m in 1:length(city.groups)){
          ...
          # file.exists("/Users/mdcc/Box/MC_R/HIA/run/rates_dc/stroke_25_2014_dc.tif") # check whether file exists in folder
          ...
          af <- mask(af, shp)
          #==============================================================================================================================================================    
          # f1 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/stroke/adults/af/',names(city.groups[m]),names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f1 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/stroke/af/",names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' AF Stroke and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('af')
          ...
          # f2 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/stroke/adults/mr/',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f2 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/stroke/mr/",names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' MR Stroke and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('mr')
          ...
          # f3 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/stroke/adults/paf/',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f3 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/stroke/paf/",names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
        ...
        paf_plot <- ggplot()  +
          ...
          ggsave(paste(names(city.groups[m]),' PAF Stroke and PM25 excess cases, ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('hia')
...
#            f4 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/stroke/hia/",names(clips.shp[s]),' ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.tif',sep='')
#            writeRaster(rate_df, filename=f4, format="GTiff", overwrite=TRUE)
          ...
          e_rate2 <- ggplot()  +
            ...
            geom_path(data = clips.shp.f, aes(x = long, y = lat, group = group),
                      color = "grey60", size = 0.5) +
            # labs(title=bquote('Stroke')) #,
            labs(title=paste0('Stroke'),
            ...
            plot.subtitle = element_text(face = "italic", hjust = 0.5, size = 13))
                 ...
                 # subtitle=paste0('Range: ',rate.min.label,' to ',rate.max.label,' per 100,000. '),sep='')
          print(e_rate2)
          ggsave(paste('Stroke_composite_',names(clips.shp[s]),names(beta.groups[i]),'_',names(rate.groups[k]),'.png',sep=''),dpi=300)
#          ggsave(paste('Stroke_composite_',names(clips.shp[s]),names(beta.groups[i]),'_',names(rate.groups[k]),'.tiff',sep=''),dpi=300)

          # require(ggplot2)
          # require(dplyr)
          # f5 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/fig1/",'Stroke_fig1','_',names(clips.shp[s]),'_',names(beta.groups[i]),'_',names(rate.groups[k]),'.tif',sep='')
