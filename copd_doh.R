# ==========================================================================================================================================
#
# HIA, DC, COPD, DOH data
# *** only need to change the date in 4 places and rerun the loops *** 
# Created: 08032020
# Note: run all every year using the PM and BDR datasets for that year, and also the 2018 PM
# 
# ==========================================================================================================================================
...
beta.groups <- c(0.00953101798) #, 0.0009950330853, 0.01739533071)  
  names(beta.groups) <- c('PE_copd') #,'LCI_copd','UCI_copd')   #,'LCI','UCI')

rate.groups <- c('copd_15_doh')              # CHANGE YEAR
  names(rate.groups) <- c('COPD_15')         # CHANGE YEAR
conc.groups <- c('vdk_2015_dc')             # CHANGE YEAR
  names(conc.groups) <- c('DC_15')          # CHANGE YEAR

pdf(NULL)

getwd()
setwd("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/copd_doh/")
    ...
    for (k in 1:length(rate.groups)){
      ...
      for (h in 1:length(pop.groups)){
        ...
        for (m in 1:length(city.groups)){
          ...
          # file.exists("/Users/mdcc/Box/MC_R/HIA/run/rates_dc/copd_25_2015_dc.tif") # check whether file exists in folder
          ...
          #==============================================================================================================================================================    
          # f1 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/copd_doh/adults/af/',names(city.groups[m]),names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f1 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/copd_doh/af/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' AF COPD and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('af')
          ...
          print(paste(names(city.groups[m]),' Mortality Risk ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(rate.groups[k]),sep=''))
          print(df.mr)
          
          f2 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/copd_doh/mr/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' MR COPD and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('mr')
          ...
          # f3 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/copd_doh/adults/paf/',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f3 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/copd_doh/paf/",names(beta.groups[i]),', ',names(conc.groups[j]),', ',names(rate.groups[k]),', ',names(pop.groups[h]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          print(paste(names(city.groups[m]),' excess mortality ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),sep=''))
          print(df.hia)
          ...
          ggsave(paste(names(city.groups[m]),' PAF COPD and PM25 excess cases, ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(pop.groups[h]),' ',names(rate.groups[k]),'.png',sep=''),dpi=300)
          print('hia')
          ...
          for (s in 1:length(clip.groups)){
            ...
            e_rate <- ggplot()  +
              ...
              geom_path(data = clips.shp.f, aes(x = long, y = lat, group = group),
                        color = "grey60", size = 0.3)+
              labs(title=bquote('COPD Excess Mortality Attributable to '~PM[2.5]~' (2015)'),
                   caption=paste0(names(beta.groups[i]),', ',names(conc.groups[j]),', \n',names(rate.groups[k]),', ',names(pop.groups[h]),'.',sep=''),
            ...
            print(e_rate3)
            ggsave(paste('COPD_comp_color ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(rate.groups[k]),'.png',sep=''),
            ...
            e_rate2 <- ggplot() +
              ...
              geom_path(data = clips.shp.f, aes(x = long, y = lat, group = group),
                        color = "grey60", size = 0.5) +
              # labs(title=bquote('COPD')) #,
              labs(title=paste0('COPD (2015)'),
                   subtitle=paste0('Range: ',rate.min.label,' - ',rate.max.label,' per 100,000'),sep='') + 
              theme (plot.title = element_text(face="bold", hjust = 0.5, size = 23),
                     plot.subtitle = element_text(face = "italic", hjust = 0.5, size = 21))
            print(e_rate2)
            ggsave(paste('COPD_composite ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(rate.groups[k]),'.png',sep=''),dpi=300)
            print("COPD")
          }
