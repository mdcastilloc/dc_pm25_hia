# ==========================================================================================================================================
#
# HIA, DC, COPD, 2011-2015 average PM2.5, pop and Rates/counts
# replace PM2.5 dataset with that of the 5-year average (for copd 2011-2015)
...
# update filepath for outputs with /copd_five/
...
pop.groups <- c('pop_30_div')
  names(pop.groups) <- c('GPW_30')
city.groups <- ('dc_boundary')
  names(city.groups) <- ('DC')
clip.groups <- c('png_dc') #,'ward_dc','tract_dc')
  names(clip.groups) <- c('PNGs') #,'Wards','Tracts')
beta.groups <- c(0.00953101798) 
  names(beta.groups) <- c('PE_copd')            #, 'LCI_ast', 'UCI_ast') #,'LCI','UCI')
count.groups <- c('copd_11_15_doh')             # update to 5-year average counts
  names(count.groups) <- c('COPD_11_15_DOH')    
conc.groups <- c('vdk_11_15_dc')             # update for 5-year average: 2011 - 2015
  names(conc.groups) <- c('DC_11-15')          


pdf(NULL)

getwd()
setwd("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/copd_doh/copd_five_ave/")
    ...
    for (k in 1:length(count.groups)){
      ...
      for (h in 1:length(pop.groups)){   # note: remove the '}' at the end 
        ...
        for (m in 1:length(city.groups)){
          ...
          mr <- (af2 * b * 100000 / c)    # betas*conc * rates (counts * 100,000/pop) 
          mr[mr==0]<-NA
          ...
          #==============================================================================================================================================================    
          # f1 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/copd_doh/adults/af/',names(city.groups[m]),names(conc.groups[j]),', ',names(count.groups[k]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f1 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/copd_doh/copd_five_ave/af/",names(conc.groups[j]),', ',names(count.groups[k]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          ggsave(paste(names(city.groups[m]),' AF COPD and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(count.groups[k]),'.png',sep=''),
          ...
          print(paste(names(city.groups[m]),' Mortality Risk ',names(conc.groups[j]),' ',names(beta.groups[i]),' ',names(count.groups[k]),sep=''))
          print(df.mr)
          
          f2 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/copd_doh/copd_five_ave/mr/",names(conc.groups[j]),', ',names(count.groups[k]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          mr_plot <- ggplot()  +
            ...
            scale_fill_gradient2("Mortality Risk per 100,000",
                                 low = "#3ec267", 
                                 mid = "#fff429",  #ff7e29
                                 high = "#fc0339", ##ff1f40
                                 midpoint = mean.mr,
            ...
            labs(title='Mortality Risk',
                 ...
                 subtitle=paste0('Range: ',min.mr.label,' to ',max.mr.label,' per 100,000. '),sep='')
          ggsave(paste(names(city.groups[m]),' MR COPD and PM25 ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(count.groups[k]),'.png',sep=''),
                 width = 8, height = 10, dpi = 300, units = "in")
          print('mr')
          ...
          # f3 = paste('/GWSPH/home/mdcastillo/results/dc/pm25/copd_doh/adults/paf/',names(conc.groups[j]),', ',names(count.groups[k]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          f3 = paste("/Users/mdcc/Box/dc_hia/HIA_calculations_mc/results/copd_doh/copd_five_ave/paf/",names(conc.groups[j]),', ',names(count.groups[k]),', ',names(beta.groups[i]),', ',names(city.groups[m]),'.tif',sep='')
          ...
          print(paste(names(city.groups[m]),' excess mortality ',names(conc.groups[j]),' ',names(beta.groups[i]),' ',names(count.groups[k]),sep=''))
          print(df.hia)
          ...
          paf_plot <- ggplot()  +
            ...
            geom_path(data = shp.f, aes(x = long, y = lat, group = group), 
                      color = "grey60", size = 0.5)+
            labs(title=bquote('COPD Excess Mortality Attributable to '~PM[2.5]~''),
          ...
          ggsave(paste(names(city.groups[m]),' PAF COPD and PM25 excess cases, ',names(beta.groups[i]),' ',names(conc.groups[j]),' ',names(count.groups[k]),'.png',sep=''),
                 width = 8, height = 10, dpi = 300, units = "in")
          print('hia')
          ...
          for (s in 1:length(clip.groups)){
            ...
            print(e_rate3)
            ggsave(paste('COPD_comp_color ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(count.groups[k]),'.png',sep=''),
                   width = 8, height = 10, dpi = 300, units = "in")
            print("AST")
            ...
            e_count <- ggplot() +
              ...
              theme (plot.title = element_text(face="bold", hjust = 0.5, size = 23),
                     ...
                     legend.title = element_text(size=18), legend.text = element_text(size=18))
            print(e_count)
            ggsave(paste('COPD_comp_count ',names(clips.shp[s]),', ',names(conc.groups[j]),', ',names(beta.groups[i]),', ',names(count.groups[k]),'.png',sep=''),
                   width = 8, height = 10, dpi = 300, units = "in")
            print("comp_counts")
